...
public class TriggerToPlayer: MonoBehaviour
{
    ...
    private bool CheckObstacleBetweenPlayerAndEnemy()
    {
        ...
        if (Physics.Raycast(transform.position, (posTarget - transform.position).normalized * Vector3.Distance(transform.position, _target.transform.position), out hit) && hit.collider.GetComponent<Player>())
        {
            return false;
        }
        return true;
